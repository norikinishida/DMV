    ...
    def parse(self, postags, model):
        ...
        for i in range(0, n_postags):
            ...
            chart[i, i, "L_s0", i] \
                    = model.get_stop_prob(head=postags[i],
                                          ...
                                          valency=0,
                                          stop=STOP)
            back_ptr[i, i, "L_s0", i] = [None, None, None, None, None, None]
            # "L_s1[h] -> h"
            if i >= 1:
                chart[i, i, "L_s1", i] \
                        = model.get_stop_prob(head=postags[i],
                                              ...
                                              valency=1,
                                              stop=STOP)
                back_ptr[i, i, "L_s1", i] = [None, None, None, None, None, None]
            ...
            while added:
                ...
                score = model.get_stop_prob(head=postags[i],
                                            ...
                                            stop=STOP) \
                         * chart[i, i, "L_s0", i]
                if chart[i, i, "R_s0", i] < score:
                    chart[i, i, "R_s0", i] = score
                    back_ptr[i, i, "R_s0", i] = [i, "L_s0", i]
                    added = True
                # "R_s1[h] -> L_s0[h]"
                if i < n_postags-1:
                    score = model.get_stop_prob(head=postags[i],
                                                ...
                                                stop=STOP) \
                              * chart[i, i, "L_s0", i]
                    if chart[i, i, "R_s1", i] < score:
                        chart[i, i, "R_s1", i] = score
                        back_ptr[i, i, "R_s1", i] = [i, "L_s0", i]
                        added = True
                ...
                for i2 in range(i1, i3):
                    for l_i in range(i1, i2+1):
                        for r_i in range(i2+1, i3+1):
                            # Left attachment "L_a[r] -> R_s0[l] L_s1[r]
                            score = model.get_attach_prob(head=postags[r_i],
                                                         direction=LEFT,
                                                         dep=postags[l_i]) \
                                   * chart[i1, i2, "R_s0", l_i] \
                                   * chart[i2+1, i3, "L_s1", r_i]
                            if chart[i1, i3, "L_a", r_i] < score:
                                chart[i1, i3, "L_a", r_i] = score
                                back_ptr[i1, i3, "L_a", r_i] = [r_i, "R_s0", l_i, "L_s1", r_i, i2]

                            # Right attachment "R_a[l] -> R_s1[l] R_s0[r]"
                            score = model.get_attach_prob(head=postags[l_i],
                                                         direction=RIGHT,
                                                         dep=postags[r_i]) \
                                   * chart[i1, i2, "R_s1", l_i] \
                                   * chart[i2+1, i3, "R_s0", r_i]
                            if chart[i1, i3, "R_a", l_i] < score:
                                chart[i1, i3, "R_a", l_i] = score
                                back_ptr[i1, i3, "R_a", l_i] = [l_i, "R_s1", l_i, "R_s0", r_i, i2]
                ...
                while added:
                    ...
                    for h_i in range(i1, i3+1):
                        ...
                        score = model.get_stop_prob(head=postags[h_i],
                                                    ...
                                                    stop=CONT) \
                                * chart[i1, i3, "L_a", h_i]
                        if chart[i1, i3, "L_s0", h_i] < score:
                            chart[i1, i3, "L_s0", h_i] = score
                            back_ptr[i1, i3, "L_s0", h_i] = [h_i, "L_a", h_i]
                            added = True
                        # "L_s1[h] -> L_a[h]"
                        score = model.get_stop_prob(head=postags[h_i],
                                                    direction=LEFT,
                                                    ...
                                                    stop=CONT) \
                                * chart[i1, i3, "L_a", h_i]
                        if chart[i1, i3, "L_s1", h_i] < score:
                            chart[i1, i3, "L_s1", h_i] = score
                            back_ptr[i1, i3, "L_s1", h_i] = [h_i, "L_a", h_i]
                            added = True
                        ...
                        score = model.get_stop_prob(head=postags[h_i],
                                                    ...
                                                    valency=0,
                                                    stop=STOP) \
                                * chart[i1, i3, "L_s0", h_i]
                        if chart[i1, i3, "R_s0", h_i] < score:
                            chart[i1, i3, "R_s0", h_i] = score
                            back_ptr[i1, i3, "R_s0", h_i] = [h_i, "L_s0", h_i]
                            added = True
                        # "R_s1[h] -> L_s0[h]"
                        score = model.get_stop_prob(head=postags[h_i],
                                                    direction=RIGHT,
                                                    ...
                                                    stop=STOP) \
                                * chart[i1, i3, "L_s0", h_i]
                        if chart[i1, i3, "R_s1", h_i] < score:
                            chart[i1, i3, "R_s1", h_i] = score
                            back_ptr[i1, i3, "R_s1", h_i] = [h_i, "L_s0", h_i]
                            added = True
                        ...
                        if chart[i1, i3, "R_s0", h_i] < score:
                            chart[i1, i3, "R_s0", h_i] = score
                            back_ptr[i1, i3, "R_s0", h_i] = [h_i, "R_a", h_i]
                            added = True
                        # "R_s1[h] -> R_a[h]"
                        score = model.get_stop_prob(head=postags[h_i],
                                                    direction=RIGHT,
                        ...
                        if chart[i1, i3, "R_s1", h_i] < score:
                            chart[i1, i3, "R_s1", h_i] = score
                            back_ptr[i1, i3, "R_s1", h_i] = [h_i, "R_a", h_i]
                            added = True
        ...
        for h_i in range(0, n_postags):
            score = model.get_root_prob(dep=postags[h_i]) \
                    * chart[0, n_postags-1, "R_s0", h_i]
            chart[0, n_postags-1, "S", h_i] = score
            back_ptr[0, n_postags-1, "S", h_i] = [h_i, "R_s0", h_i]

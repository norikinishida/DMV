...
class DMwoV(object):
                            ...
                            for r_i in range(i2+1, i3+1):
                                ...
                                attach_counts[postags[l_i], RIGHT, postags[r_i]] \
                                        += self.get_attach_prob(head=postags[l_i],
                                            ...
                                            * inside[i1, i2, "R_s", l_i] \
                                            * inside[i2+1, i3, "R_s", r_i] \
                                            / inside[0, n_postags-1, "S", -1]

            # Stop Rules / Unary Rules
            # Count stop(h, <-): L_s[h] -> h
    ...
    def compute_inside_prob(self, postags):
        ...
        for i in range(0, n_postags):
            # "L_s[h] -> h"
            inside[i, i, "L_s", i] += self.get_stop_prob(head=postags[i],
                                                          direction=LEFT,
                                                          valency=0,
                                                          stop=STOP)
            ...
            # Handle unaries
            # "R_s[h] -> L_s[h]"
            inside[i, i, "R_s", i] += self.get_stop_prob(head=postags[i],
                                                          direction=RIGHT,
                                                          valency=0,
                                                          stop=STOP) \
                                        * inside[i, i, "L_s", i]
        ...
        for d in range(1, n_postags):
            for i1 in range(0, n_postags - d):
                ...
                for h_i in range(i1, i3+1):
                    ...
                    # "L_s[h] -> L_a[h]"
                    inside[i1, i3, "L_s", h_i] += self.get_stop_prob(head=postags[h_i],
                                                                      direction=LEFT,
                                                                      valency=0,
                                                                      stop=CONT) \
                    ...
                    # "R_s[h] -> L_s[h]"
                    inside[i1, i3, "R_s", h_i] += self.get_stop_prob(head=postags[h_i],
                                                                      direction=RIGHT,
                                                                      valency=0,
                                                                      stop=STOP) \
                                                    * inside[i1, i3, "L_s", h_i]
    ...
    def compute_outside_prob(self, postags, inside):
        ...
        for i in range(0, n_postags):
            ...
            # Handle unaries
            # "S -> R_s[h]"
            outside[0, n_postags-1, "R_s", i] += self.get_root_prob(dep=postags[i]) \
                                                  * outside[0, n_postags-1, "S", i]
            # "R_s[h] -> L_s[h]"
            outside[0, n_postags-1, "L_s", i] += self.get_stop_prob(head=postags[i],
                                                                     direction=RIGHT,
                                                                     valency=0,
                                                                     stop=STOP) \
                                                    * outside[0, n_postags-1, "R_s", i]
            # "L_s[h] -> L_a[h]"
                ...
                for i0 in range(0, i1):
                    for l_i in range(i0, i1):
                        for r_i in range(i1, i3+1):
                            # "R_a[l] -> R_s[l] R_s[r]"
                            outside[i1, i3, "R_s", r_i] += self.get_attach_prob(head=postags[l_i],
                                                                                 direction=RIGHT,
                                                                                 dep=postags[r_i]) \
                                                            * inside[i0, i1-1, "R_s", l_i] \
                                                            * outside[i0, i3, "R_a", l_i]
                            # "L_a[r] -> R_s[l] L_s[r]
                            outside[i1, i3, "L_s", r_i] += self.get_attach_prob(head=postags[r_i],
                                                                                 direction=LEFT,
                                                                                 dep=postags[l_i]) \
                                                            * inside[i0, i1-1, "R_s", l_i] \
                ...
                for i4 in range(i3+1, n_postags):
                    for l_i in range(i1, i3+1):
                        for r_i in range(i3+1, i4+1):
                            # "R_a[l] -> R_s[l] R_s[r]"
                            outside[i1, i3, "R_s", l_i] += self.get_attach_prob(head=postags[l_i],
                                                                                 direction=RIGHT,
                                                                                 dep=postags[r_i]) \
                                                            * inside[i3+1, i4, "R_s", r_i] \
                                                            * outside[i1, i4, "R_a", l_i]
                            # "L_a[r] -> R_s[l] L_s[r]
                            outside[i1, i3, "R_s", l_i] += self.get_attach_prob(head=postags[r_i],
                                                                                 direction=LEFT,
                                                                                 dep=postags[l_i]) \
                                                            * inside[i3+1, i4, "L_s", r_i] \
                                                            * outside[i1, i4, "L_a", r_i]

                # Handle unaries
                for h_i in range(i1, i3+1):
                    # "R_s[h] -> L_s[h]"
                    outside[i1, i3, "L_s", h_i] += self.get_stop_prob(head=postags[h_i],
                                                                       direction=RIGHT,
                                                                       valency=0,
                                                                       stop=STOP) \
                                                    * outside[i1, i3, "R_s", h_i]
                    # "R_s[h] -> R_a[h]"
